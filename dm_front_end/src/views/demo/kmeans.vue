...
export default {
  ...
  methods: {
    ...
    showChart() {
      // axios.get('http://localhost:8080/api/page?pageNum=' + this.pageNum + '&pageSize=' + this.pageSize).then(res => {
      //   this.pieces_raw_data = res.data.list
      ...
      for (var i = 0; i < this.pieces_raw_data.length; i++) {
        this.pieces_raw_data.color = COLOR_ALL[this.pieces_raw_data.label]
      }
      pieces = this.pieces_raw_data
      const option = {
        dataset: [
          {
            source: pieces
